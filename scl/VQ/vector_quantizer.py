...
def build_kmeans_codebook(
    ...
    clusters = np.zeros((len(data_npy_2d[:,0])))

    #####################################################
    # ADD DETAILS HERE
    ######################################################

    codebook = data_npy_2d[np.random.choice(len(data_npy_2d),num_vectors,replace=False)]

    for iter in range(max_iter):
        for vec in range(len(data_npy_2d[:,0])):
            min_ind, dist = find_nearest(codebook,data_npy_2d[vec,:],mse)
            clusters[vec] = min_ind

        new_codebook = np.array([data_npy_2d[clusters == i].mean(axis=0) for i in range(len(np.unique(clusters)))])
...
def _vq_experiment(data_npy, codebook_npy):
    ...
    max_diff = abs(data_npy-decoded_data_npy).max()
    return avg_bits, avg_mse_distortion, max_diff, decoded_data_npy
...
def quantize_integrals():
    ...
    print("Avg mse: ", avg_mse_distortion) 
